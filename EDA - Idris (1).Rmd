...
<li><a href="#Structure">Exploratory Data Analysis</a></li>
<ul>
...
<li><a href="#Predictive_modelling">Predictive Modelling</a></li>
...
<li><b>Prevalence</b> is the proportion of a population who have a specific characteristic in a given population. It is calculated as : Prevalence = (TP + TN)/(TP + TN + FP + FN)</li>
<li><b>Detection rate</b> is the proportion of the whole sample where the events were detected as True positives. Detection Rate = TP /(TP + FP + TN + FN)</li>
...
logistic_sig<-glm(formula = HeartDisease ~ sex + CPType + restBP  + 
...
logistic_sig <- train(HeartDisease ~ sex + CPType + restBP +  
...
LR_Models<-data.frame(Model=c("Using all predictor variables","Using Backward Propogation","Using only the significant features "),Training_Accuracy=c(87.6,85.4,83.1),Testing_Accuracy =c(83.4,77.1,82.1),Sensitivity=c(69.2,58.9,66.6),Specificity=c(97.6,95.2,97.6))
...
```{r,echo=F}
model_hd1 <- rpart(HeartDisease ~ sex + CPType + restBP  + restECG+ ExIndAngina + nVessels+slopePEST, data=train_set)
...
heart_rf=randomForest(HeartDisease ~ sex + CPType + restBP  + restECG+ ExIndAngina + nVessels+slopePEST , data = train_set)
heart_rf
#heart_rf$importance
```
...
```{r,echo=F}
model_cv_rf <- train(HeartDisease ~ sex + CPType + restBP  + restECG+ ExIndAngina + nVessels+slopePEST,data=heart,method="rf",trControl = trainControl(method = "cv",number = 3))
model_cv_rf
```
...
```{r,echo=F}
model_cv_logistic <- train(HeartDisease ~ sex + CPType + restBP  + restECG+ ExIndAngina + nVessels+slopePEST,data=heart,method="glm",family="binomial",trControl = trainControl(method = "cv",number = 5))
model_cv_logistic
```
...
The k-fold cross validation method involves splitting the dataset into k-subsets. For each subset is held out while the model is trained on all other subsets. This process is completed until accuracy is determine for each instance in the dataset, and an overall accuracy estimate is provided. It is a robust method for estimating accuracy. Here we have taken k=5.
```{r,echo=F}
model_cv_dt <- train(HeartDisease ~ sex + CPType + restBP  + restECG+ ExIndAngina + nVessels+slopePEST,data=heart,method="rpart",trControl = trainControl(method = "cv",number = 5))
model_cv_dt
```
...
```{r,echo=F}
Models<-data.frame(Model=c("Logistic Regression","Decision Tree","Random Forest"),Training_Accuracy=c(83.1,85.3,80.5),Testing_Accuracy =c(82.1,77.4,78.5),Cross_Validation_Acuracy=c(83.3,73.7,80))
